...
{
    public partial class planeWithThirtySeats : Form
    {
        ...
        public planeWithThirtySeats()
        {
            InitializeComponent();
        }

        private void planeWithThirtySeats_Load(object sender, EventArgs e)
        {
            this.reloadSeats();
        ...
        public void reloadSeats()
        {
            if (!reversed)
            {
                int dI = this.ts.airplane.road.roadwayString.IndexOf(this.ts.route.departure.name);
                int aI = this.ts.airplane.road.roadwayString.IndexOf(this.ts.route.arrival.name);

                Dictionary<List<string>, List<int>> currentSeats = ts.airplane.dailySeats[ts.day];
                ...
                for (int i = 1; i <= ts.airplane.seat_count; i++)
                {
                    founds[i] = false;
                }
                ...
                foreach (KeyValuePair<List<string>, List<int>> seats in currentSeats)
                {
                    ...
                    {
                        for (int i = 0; i < ts.airplane.seat_count; i++)
                        {
                            if (seats.Value[i] == 1)
                            {
            ...
            {
                int dI = this.ts.airplane.road.reversedRoadwayString.IndexOf(this.ts.route.departure.name);
                int aI = this.ts.airplane.road.reversedRoadwayString.IndexOf(this.ts.route.arrival.name);

                Dictionary<List<string>, List<int>> currentSeats = ts.airplane.reversedDailySeats[ts.day];
        ...
        private void biletAl()
        {
            ...
            if (!reversed)
            {
                dI = this.ts.airplane.road.roadwayString.IndexOf(this.ts.route.departure.name);
                aI = this.ts.airplane.road.roadwayString.IndexOf(this.ts.route.arrival.name);
            }
            else
            {
                dI = this.ts.airplane.road.reversedRoadwayString.IndexOf(this.ts.route.departure.name);
                aI = this.ts.airplane.road.reversedRoadwayString.IndexOf(this.ts.route.arrival.name);
            }
        ...
        public static List<List<City>> FindCombinations(List<City> dataList)
        {
            int n = dataList.Count;

            List<List<City>> cityComb = new List<List<City>>();
        ...
        {
            this.buttonNumber = 29;
            optimizeButtonColours();
        }

        private void button30_Click(object sender, EventArgs e)
        {
            this.buttonNumber = 30;
            optimizeButtonColours();
        }
        ...
        {
            this.buttonNumber = 26;
            optimizeButtonColours();
        }

        private void button21_Click(object sender, EventArgs e)
        {
            this.buttonNumber = 21;
            optimizeButtonColours();
        }
        ...
        {
            this.buttonNumber = 18;
            optimizeButtonColours();
        }

        private void button19_Click(object sender, EventArgs e)
        {
            this.buttonNumber = 19;
            optimizeButtonColours();
        }
        ...
        {
            if (numberOfPeople >= ts.airplane.seat_count)
            {
                MessageBox.Show("En fazla " + ts.airplane.seat_count + " adet kişi girebilirsiniz!");
            }
            else
            {
                numberOfPeople++;
