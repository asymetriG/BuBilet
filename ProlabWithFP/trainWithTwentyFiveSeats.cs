...
{
    public partial class trainWithTwentyFiveSeats : Form
    {
        ...
        Customer costumer;
        public trainWithTwentyFiveSeats()
        {
            InitializeComponent();
        }

        private void trainWithTwentyFiveSeats_Load(object sender, EventArgs e)
        {
            this.reloadSeats();
        ...
        public void reloadSeats()
        {
            if (!reversed)
            {
                int dI = this.ts.train.road.roadwayString.IndexOf(this.ts.route.departure.name);
                int aI = this.ts.train.road.roadwayString.IndexOf(this.ts.route.arrival.name);

                Dictionary<List<string>, List<int>> currentSeats = ts.train.dailySeats[ts.day];
                ...
                for (int i = 1; i <= ts.train.seat_count; i++)
                {
                    founds[i] = false;
                }
                ...
                foreach (KeyValuePair<List<string>, List<int>> seats in currentSeats)
                {
                    ...
                    {
                        for (int i = 0; i < ts.train.seat_count; i++)
                        {
                            if (seats.Value[i] == 1)
                            {
            ...
            {
                int dI = this.ts.train.road.reversedRoadwayString.IndexOf(this.ts.route.departure.name);
                int aI = this.ts.train.road.reversedRoadwayString.IndexOf(this.ts.route.arrival.name);

                Dictionary<List<string>, List<int>> currentSeats = ts.train.reversedDailySeats[ts.day];
        ...
        private void biletAl()
        {
            ...
            if (!reversed)
            {
                dI = this.ts.train.road.roadwayString.IndexOf(this.ts.route.departure.name);
                aI = this.ts.train.road.roadwayString.IndexOf(this.ts.route.arrival.name);
            }
            else
            {
                dI = this.ts.train.road.reversedRoadwayString.IndexOf(this.ts.route.departure.name);
                aI = this.ts.train.road.reversedRoadwayString.IndexOf(this.ts.route.arrival.name);
            }
        ...
        {
            if (numberOfPeople >= ts.train.seat_count)
            {
                MessageBox.Show("En fazla " + ts.train.seat_count + " adet kişi girebilirsiniz!");
            }
            else
            {
                numberOfPeople++;
